...
class ThreadPool {
    private:
        ...
        std::condition_variable task_inqueue_signal_;
        std::condition_variable task_finish_signal_;
        std::atomic_int task_counter_;
        bool stop_;
        std::queue<std::tuple<IRunnable*, int, int>> tasks_queue_;
    public:
        ThreadPool(int num_threads);
        ~ThreadPool();
        void Init();
        void AddTask(IRunnable* runnable, int task_id, int num_total_tasks);
        int GetFinishTaskNum();
        void WaitAllTaskFinish(int num_total_tasks);
};
